...
from pprint import pprint # Needed to print the temps in a nice way
#########
os.system("clear")
#########
def intro():
...
def askforcity():
    ...
    try:  # Try to convert to a int. if not restart
        ...
        askforcity()
    if numberofcities <= 1:
        input("You can't compare less then 2 cities silly (PRESS ENTER)")
        askforcity()
    numberoftries = 1
    my_array = []
    while numberoftries <= numberofcities:
        ...
        city = input("")
        print("Please enter the temp for city", numberoftries)
        citytemp = input("")       
        try:  # Try to convert to a int. if not restart
